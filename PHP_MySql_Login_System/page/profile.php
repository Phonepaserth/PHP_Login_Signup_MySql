...
$stmt = $con->prepare('SELECT password, status, photo, id, date_time FROM accounts WHERE id = ?');
// In this case we can use the account ID to get the account info.
$stmt->bind_param('i', $_SESSION['id']);
$stmt->execute();
$stmt->bind_result($password, $status, $photo, $id, $date);
$stmt->fetch();
$stmt->close();
if ($status == 1){
    $status = "Admin";
...
<body class="loggedin">
    <nav class="navtop">
        <div>
            <h1><a href="home.php">Website Phonepaserth Company</a></h1>
